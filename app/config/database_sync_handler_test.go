...
func TestDatabaseSyncHandlerConfigUpsert(t *testing.T) {
	...
	err := handler.OnConfigUpdate(configFile, config, false)
	if err != nil {
...
func TestDatabaseSyncHandlerConfigDeletion(t *testing.T) {
	...
	err := handler.OnConfigUpdate("/tmp/test.yml", config, true)
	if err != nil {
		t.Errorf("Expected no error, got: %v", err)
	}
...
func TestDatabaseSyncHandlerNonExistentFile(t *testing.T) {
	...
	err := handler.OnConfigUpdate("/tmp/nonexistent.yml", config, false)
	if err != nil {
